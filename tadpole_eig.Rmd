...
\label{eq:sol_reprentation}
        u(s,t) = \displaystyle\sum_{j\in\mathbb{N}}e^{-\EIGENVAL^{\frac{\alpha}{2}}_jt}\left(u_0, e_j\right)_{L_2(\Gamma)}e_j(s).
\end{equation}
...
```{r}
EIGENFUN <- apply(EIGENFUN, 2, order_to_plot)
x <- order_to_plot(x)
y <- order_to_plot(y)

# Create interactive plot
fig <- plot_ly(x = ~INDEX, y = ~EIGENVAL, type = 'scatter', mode = 'markers',
               marker = list(size = 6),
               name = "Eigenvalues")

fig <- fig %>% layout(title = "Eigenvalues",
                      xaxis = list(title = "index i"),
                      yaxis = list(title = "Eigenvalues_i"))



plot_data <- data.frame(
  x = rep(x, times = ncol(EIGENFUN)),
  y = rep(y, times = ncol(EIGENFUN)),
  z = as.vector(EIGENFUN),
  frame = rep(INDEX, each = length(x))
)

# Compute axis limits
x_range <- range(x)
y_range <- range(y)
z_range <- range(EIGENFUN)
...
for (i in seq_along(pb$x$frames)) {
  t <- INDEX[i]
  err <- signif(EIGENVAL[i], 4)
  pb$x$frames[[i]]$layout <- list(title = paste0("i: ", t, " | Eigenvalue_i: ", err))
}
