...
graph <- gets.star.graph(h = h, n = 3, length = 1)
graph$compute_fem()
G <- graph$mesh$G
C <- graph$mesh$C
L <- kappa^2*C + G
...
U_0 <- -3*cos(2 * pi * graph_coord_loc[,2])/(8*pi^2)
FF <- outer(cos(2 * pi * graph_coord_loc[, 2]), sin(2 * pi * time_seq)*0+1)

overkill_graph <- gets.star.graph(h = 0.0001, n = 3, length = 1)
overkill_graph$compute_fem()
overkill_graph_cord_loc <- overkill_graph$mesh$VtE
# Compute matrix F with columns F^k
inner_product <- t(graph$fem_basis(overkill_graph$get_mesh_locations())) %*%
  overkill_graph$mesh$C %*%
  cos(2 * pi * overkill_graph_cord_loc[, 2])
FF_approx <- outer(inner_product, sin(2 * pi * time_seq))*0
```
...
plotting.order <- function(v, graph){
  ...
  new_v <- c(v[1], red_v[which(red_edge_number == 1)], v[2], NA, 
             v[3], red_v[which(red_edge_number == 2)], v[2], NA, 
             v[4], red_v[which(red_edge_number == 3)], v[2])
  return(new_v)
}
...
graph.plotter.3d(graph, FF, time_seq, time_step)
```
