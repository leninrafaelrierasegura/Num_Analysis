...
# POWERS <- 8:0
POWERS <- seq(8, 0, by = -0.5)

time_steps <- 0.1 * 2^-POWERS
hs <- time_steps^(1/2)

# Overkill parameters
overkill_time_step <- 0.1 * 2^-10
overkill_h <- (0.1 * 2^-10)^(1/2)
...
for (i in 1:length(hs)) {
  ...
  for (j in 1:length(time_steps)) {
    ...
    time_seq <- seq(0, T_final, length.out = ((T_final - 0) / time_step + 1)) # seq(0, T_final, by = time_step)
...
```{r}
idx <- seq(1, length(overkill_time_seq), by = 512)
