...
```{r}
time_step <- 1
T_final <- time_step*20
time_seq <- seq(0, T_final, by = time_step)
h <- 0.001#time_step/100
...
- When $\alpha$ is large, we need less terms in the series to approximate the solution.

- When $\alpha$ is small, $h<\tau$ is preferable.

- When $\alpha$ is large, $h<\tau$ is preferable.
...
x <- order_to_plot(x)
y <- order_to_plot(y)
max_error_at_each_time <- apply(abs(U_true1 - U_true2), 2, max)
...
plot_data <- data.frame(
  ...
  z_true = as.vector(U_true1),
  z_approx = as.vector(U_true2),
