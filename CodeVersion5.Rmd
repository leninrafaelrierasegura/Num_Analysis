...
plotting.order <- function(v, graph){
  ...
  red_v <- v[(graph$nV+1):length(v)]
  red_edge_number <- edge_number[(graph$nV+1):length(v)]
  start_v <- v[1:graph$nV]
  start_edge_number <- edge_number[1:graph$nV]
  atzero <- which(start_edge_number == 1)
  new_v <- c(start_v[atzero], red_v[which(red_edge_number == 1)])
  for (edge in 2:graph$nE) {
    atzero <- which(start_edge_number == edge)
    new_v <- c(new_v, NA, start_v[atzero], red_v[which(red_edge_number == edge)])
  }
  return(new_v)
}
