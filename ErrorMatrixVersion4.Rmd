...
```{r, fig.align='center', fig.dim= c(18,18), fig.cap = captioner("Caption")}
errors <- errors_projected
print(errors)
df <- melt(errors)
colnames(df) <- c("tau", "alpha", "Value")

df$tau <- factor(df$tau, levels = unique(df$tau), labels = tau_vector)
df$alpha <- factor(df$alpha, levels = unique(df$alpha), labels = alpha_vector)

p <- ggplot(df, aes(x = alpha, y = tau, fill = Value, text = paste("Value:", Value))) +
...
for (u in 1:length(alpha_vector)) {
  computed_rates[u] <- coef(lm(log(errors_projected[, u]) ~ log(tau_vector)))[2]
}
guiding_lines <- compute_guiding_lines(tau_vector, alpha_vector, errors_projected, theoretical_rate = rep(1, length(alpha_vector)))
...
alpha_formatted <- formatC(alpha_vector, format = "f", digits = 1)
custom_labels <- paste0(alpha_formatted, " (", truncate_to_two(computed_rates), ")")
...
```{r}
ggsave(here::here("data_files/conv_rates_tau.png"), width = 6, height = 4, plot = p, dpi = 300)
```
