...
solve_fractional_evolution <- function(my_op_frac, time_step, time_seq, val_at_0, RHSF) {
  CC <- my_op_frac$C
  SOL <- matrix(NA, nrow = nrow(CC), ncol = length(time_seq))
  SOL[, 1] <- val_at_0
  for (k in 1:(length(time_seq) - 1)) {
    rhs <- CC %*% SOL[, k] + time_step * RHSF[, k + 1]
...
U_eval <- solve_fractional_evolution(my_op_frac, time_step, time_seq, val_at_0 = u_0, FF_plus_ZZ)
UU <- R %*% A_proj %*% U_eval
VV <- UU[, rev(seq_len(ncol(UU)))]
Q_eval <- solve_fractional_evolution(my_op_frac, time_step, time_seq, val_at_0 = u_0*0, VV - VdVd)
...
# Precompute Ud target (doesn't change per iteration)
Ud_eval <- outer(overkill_elliptic_u, psi) + 
...
# Initialization
P_eval <- matrix(0, nrow = nrow(C), ncol = length(time_seq))
Z_eval <- matrix(0, nrow = nrow(C), ncol = length(time_seq))
U_eval <- matrix(0, nrow = nrow(C), ncol = length(time_seq))
...
for (iter in 1:max_iter) {
  ...
  # 2. Solve forward problem (U)
  z_bar <- Z_eval  # assumed same name used inside
  F_eval <- outer(overkill_elliptic_u, psi_prime) + 
            outer(overkill_elliptic_f, psi) - A_proj %*% z_bar
  FF <- R %*% F_eval
  ZZ <- R %*% A_proj %*% Z_eval
  U_eval <- solve_fractional_evolution(my_op_frac, time_step, time_seq, val_at_0 = u_0, FF + ZZ)

  # 3. Prepare for backward problem (reverse U for V)
  UU <- R %*% A_proj %*% U_eval
  VV <- UU[, rev(seq_len(ncol(UU)))]

  # 4. Solve backward problem (P)
  Q_eval <- solve_fractional_evolution(my_op_frac, time_step, time_seq, val_at_0 = u_0 * 0, VV - VdVd)
  P_eval <- Q_eval[, rev(seq_len(ncol(Q_eval)))]

  # 5. Compute convergence metrics (L2 in space and time with weights)
  time_weights <- rep(time_step, ncol(P_eval))
  w_colsum <- function(M) as.double(t(weights) %*% (M^2) %*% time_weights)
  delta_P <- sqrt(w_colsum(P_eval - P_old))
  delta_U <- sqrt(w_colsum(U_eval - U_old))
  delta_Z <- sqrt(w_colsum(Z_eval - Z_old))

  # 6. Report
  cat(sprintf("Iter %d: ΔP = %.2e, ΔU = %.2e, ΔZ = %.2e\n", iter, delta_P, delta_U, delta_Z))

  # 7. Check convergence
  if (max(delta_P, delta_U, delta_Z) < tol) break
}
...
```{r}
graph.plotter.3d(graph, time_seq[idx], u_d[, idx])
