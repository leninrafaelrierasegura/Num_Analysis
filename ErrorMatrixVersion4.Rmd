...
POWERS <- seq(8.5, 6.5, by = -0.5)
tau_vector <- 0.1 * 2^-POWERS

# Overkill parameters
overkill_time_step <- 0.1 * 2^-16
overkill_h <- (0.1 * 2^-16)^(1/2)
...
m_values <- c()
alpha_vector <- seq(0.75, 1.75, by = 0.25)
...
guiding_lines <- compute_guiding_lines(tau_vector, alpha_vector, errors_projected, theoretical_rate = theoretical_rates)
...
p <- ggplot() +
  ...
  labs(
    ...
    x = expression(italic(tau)~(log[10]~scale)),
...
slackr_msg(
  text = "Convergence in tau completed",
  channel = "#ranks"
)
```
