...
errors_sliced <- matrix(NA, nrow = length(hs), ncol = length(time_steps))
errors_projected <- matrix(NA, nrow = length(hs), ncol = length(time_steps))
for (i in 1:length(hs)) {
  ...
  for (j in 1:length(time_steps)) {
    ...
    coarse_indices <- seq(1, length(overkill_time_seq), by = by_vector[j]) # select the indices
    time_seq <- overkill_time_seq[coarse_indices] # select the time sequence
    ...
    # Allocate matrix for the piecewise constant projection
    ...
    errors_sliced[i,j] <- sqrt(as.double(t(overkill_weights) %*% (sliced_overkill_U_true - projected_U_approx)^2 %*% rep(time_step, length(time_seq))))
  }
}
```
...
```{r}
idx <- seq(1, length(overkill_time_seq), by = 256)
graph.plotter.3d(overkill_graph, overkill_U_true[,idx], projected_U_piecewise[,idx], overkill_time_seq[idx], overkill_time_step)
```




```{r, fig.align='center', fig.dim= c(10,10), fig.cap = captioner("Caption")}
