...
\end{equation}
\begin{equation}
   \text{In matrix notation: } \quad\boldsymbol{U}_0 = \boldsymbol{E}_h\boldsymbol{w}, \quad  \boldsymbol{E}_h = \left[e_0, e_1, \ldots, e_{N}\right], \quad \boldsymbol{w} = \left[w_0, w_1, \ldots, w_{N}\right]^\top,
\end{equation}
\begin{equation}
...
or directly,
\begin{equation}
   \text{In } \texttt{R}: \texttt{U_true <- EIGENFUN %*% outer(1:length(coeff_U_0), 1:length(time_seq), function(i, j) (coeff_U_0[i] + coeff_FF[i]*time_seq[j]) * exp(-EIGENVAL_ALPHA[i] * time_seq[j]))}
\end{equation}

This is how matrix $\boldsymbol{F} = [F^0, \dots, F^K]$ is computed. This matrix has elements $\boldsymbol{F}_{ik} =(f(\cdot, t_k), \psi_i)_{L_2(\Gamma)}=\sum_{j=0}^Nv_je^{-\lambda^{\alpha/2}_jt_k}(e_j, \psi_i)_{L_2(\Gamma)}$
