...
for (k in 1:(length(time_seq) - 1)) {
  RHS <- Pr%*% (U_mat[, k] + time_step * funF[, k + 1])
...
p_f$x$layout$scene$xaxis$range <- range(y)
p_f$x$layout$scene$yaxis$range <- range(x)
p_f$x$layout$scene$zaxis$range <- range(fun_mat)
p_f
# Plot the movie of the solution
p_sol <- graph$plot_movie(U_mat)
p_sol$x$layout$scene$xaxis$range <- range(y)
p_sol$x$layout$scene$yaxis$range <- range(x)
