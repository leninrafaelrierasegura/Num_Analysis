...
POWERS <- seq(13, 8, by = -1) #13-8

time_steps <- 0.1 * 2^-POWERS
h_vector <- time_steps^(1/2)

# Overkill parameters
overkill_time_step <- 0.1 * 2^-14 #14
overkill_h <- (0.1 * 2^-14)^(1/2) #14
overkill_m <- 4

# Finest time and space mesh
overkill_time_seq <- seq(0, T_final, length.out = ((T_final - 0) / overkill_time_step + 1))
overkill_graph <- gets.star.graph(h = overkill_h, n = 3, length = 1)
overkill_graph$compute_fem()
overkill_G <- overkill_graph$mesh$G
overkill_C <- overkill_graph$mesh$C
overkill_L <- kappa^2*overkill_C + overkill_G

overkill_graph_coord_loc <- overkill_graph$mesh$VtE
overkill_U_0 <- -cos(2 * pi * overkill_graph_coord_loc[,2])/(4*pi^2)
...
for (j in 1:length(alpha_vector)) {
  ...
  my_op_frac <- my.fractional.operators.frac(overkill_L, beta, overkill_C, scale.factor = kappa^2, m = overkill_m, overkill_time_step)
  overkill_U_true <- matrix(NA, nrow = nrow(overkill_C), ncol = length(overkill_time_seq))
  overkill_U_true[, 1] <- overkill_U_0
  # Time-stepping loop
  for (k in 1:(length(overkill_time_seq) - 1)) {
    overkill_U_true[, k + 1] <- as.matrix(my.solver.frac(my_op_frac, my_op_frac$C %*% overkill_U_true[, k]))
  }
...
print(m_values)
save(errors_projected, file = here::here("data_files/errors_projected_h_star.RData"))
```
...
```{r}
ggsave(here::here("data_files/conv_rates_h_star.png"), width = 4, height = 5, plot = p, dpi = 300)
```


```{r}
library(slackr)
token = "xoxb-8768832357510-8768852406294-t9F8Dyjup5VIcHJd1Y3c4dZL"
slackr_setup(token = token)
slackr_msg(
  text = "Convergence in h completed",
  channel = "#ranks"
)
```
