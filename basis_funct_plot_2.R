...
graph <- gets.graph.basis(h = 1/2, cont = TRUE)
graph_disc <- gets.graph.basis(h = 1/20, cont = FALSE)
graph_cont <- gets.graph.basis(h = 1/20, cont = TRUE)

# discontinuous mesh
V <- graph_disc$mesh$V
VtE <- graph_disc$mesh$VtE


new_VtE <- add_group_boundaries(VtE)
new_V <- graph_disc$coordinates(PtE = new_VtE, normalized = TRUE)

A <- graph$fem_basis(new_VtE)
A_with_zero <- as.matrix(cbind(A, rep(0, nrow(A))))

V_with_NA <- rbind(c(NA,NA), insert_na_between_groups(new_V, new_VtE[,1]))
x <- V_with_NA[,1]
y <- V_with_NA[,2]

A_with_NA <- rbind(rep(NA, ncol(A_with_zero)), insert_na_between_groups(A_with_zero, new_VtE[,1]))
A_with_NA_cleaned <- apply(A_with_NA, 2, fill_between_NA)
A_with_NA_zeroed <- apply(A_with_NA, 2, keep_nonzeros_and_border_zeros)

A_with_NA_cleaned[, ncol(A_with_NA_cleaned)] <- A_with_NA[, ncol(A_with_NA)]
A_with_NA_zeroed[, ncol(A_with_NA_zeroed)] <- A_with_NA[, ncol(A_with_NA)]
...
# Add function lines
for (i in 1:ncol(A_with_NA_zeroed)) {
  z <- A_with_NA_zeroed[, i]
  if (i%in% 1:graph$nV) {
    mycolor <- "red"
  } else {
    mycolor <- "blue"
  }
  if (i==ncol(A_with_NA_zeroed)) {
    mycolor <- "black"
  } 
  # Add basis function trace
  p <- add_trace(p, x = x, y = y, z = z, type = "scatter3d", mode = "lines", 
                 name = paste("Basis function", i), showlegend = FALSE,line = list(color = mycolor, width = 2))
