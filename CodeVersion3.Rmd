...
# g_linear(r = time_seq[j], A = AAA, lambda_j_alpha_half = EIGENVAL_ALPHA[i])
# G_linear(t = time_seq[j], A = AAA)
# g_exp(r = time_seq[j], A = AAA, mu = MU)
# G_exp(t = time_seq[j], A = AAA, lambda_j_alpha_half = EIGENVAL_ALPHA[i], mu = MU)
# g_poly(r = time_seq[j], A = AAA, n = NNN)
# G_poly(t = time_seq[j], A = AAA, lambda_j_alpha_half = EIGENVAL_ALPHA[i], n = NNN)
# g_sin(r = time_seq[j], A = AAA, omega = OMEGA)
# G_sin(t = time_seq[j], A = AAA, lambda_j_alpha_half = EIGENVAL_ALPHA[i], omega = OMEGA)
# g_cos(r = time_seq[j], A = AAA, theta = THETA)
# G_cos(t = time_seq[j], A = AAA, lambda_j_alpha_half = EIGENVAL_ALPHA[i], theta = THETA)
...
FF_approx <- t(graph$fem_basis(overkill_graph$get_mesh_locations())) %*%
...
# Compute the true f
FF <- EIGENFUN %*% outer(1:length(coeff_FF), 
                         1:length(time_seq), 
                         function(i, j) coeff_FF[i] * g_sin(r = time_seq[j], A = AAA, omega = OMEGA))
...
graph.plotter.3d(graph, FF, FF_approx, FF_approx, time_seq, time_step)
```



```{r, fig.dim = c(12,6)}
idx <- seq(1, length(time_seq), by = 4)
graph.plotter.3d.comparer(graph, U_true[,idx], U_approx1[,idx], time_seq[idx])
graph.plotter.3d.single(graph, FF[, idx], time_seq[idx])
```
