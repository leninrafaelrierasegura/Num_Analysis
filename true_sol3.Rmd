...
N_finite <- 10 # choose an even number
...
x <- order_to_plot(x)
y <- order_to_plot(y)
max_error_at_each_time <- apply(abs(U_true - U_true2)/abs(U_true), 2, max)
...
x_range <- range(x)
y_range <- range(y)
z_range <- range(c(U_true, U_true2))
